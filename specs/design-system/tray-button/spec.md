...
The component's visual presentation (minimized vs. open) is dictated by the state of its parent container (the AppTray).
- When the parent tray is marked as minimized (collapsed), the button hides its text label, displaying only the centered icon.
- When the parent tray is expanded, the button displays both its icon and the text label.
...
| `src/styles/tray-button.css` | Flexbox layout, interaction states, and parent-dependent responsive hiding logic for the label. |

### Anti-Patterns

- **Hardcoded JavaScript Resize Checks**: The component should not observe window width via JavaScript to toggle the label. Rely on CSS logic (e.g., CSS Container Queries or a parent `.app-tray[data-state="minimized"]` selector).
...
Scenario: Minimized Tray State (Desktop/Tablet)
  Given: The AppTray is collapsed to its minimized state
  When: The user views the Tray Button
  Then: Only the icon is visible, centered within the button's reduced width, and the text label is fully hidden.
